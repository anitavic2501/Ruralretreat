...
function getAdminUsers(){
	global $conn, $roles;
	$sql = "SELECT username, name, email, r.id rid, u.id uid from users u left join roles r on r.id = u.role_id where u.id is not null";
...
function createAdmin($request_values){
	...
	if (count($errors) == 0) {
		...
		$query = "INSERT INTO users (username, role_id, email, password, created_at, updated_at) 
				  VALUES('$username','$role','$email','$password', now(), now())";
		mysqli_query($conn, $query);
...
function updateAdmin($request_values){
	...
	if (count($errors) == 0) {
		...
		$query = "UPDATE users SET username='$username', role_id='$role', email='$email', password='$password' WHERE id=$admin_id";
		mysqli_query($conn, $query);
