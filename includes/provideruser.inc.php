...
if (isset($_POST['provideruser-submit'])) {
  ...
  if (empty($username) || empty($email) || empty($password) || empty($passwordRepeat)) {
    	echo "<script> alert('One or more fields are empty!!');window.location='../provideruser.php?username=".$username."&email=".$email."'</script>";
         exit();
  }
  // We check for an invalid username AND invalid e-mail.
   else if (!filter_var($email, FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z0-9]*$/", $username)){
  echo "<script> alert('Invalid Email and username!!');window.location='../provideruser.php'</script>";
         exit();
  }
  // We check for an invalid e-mail.
  else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
  echo "<script> alert('Invalid Email!!');window.location='../provideruser.php?username=".$username."'</script>";
         exit();
  }
  // We check for an invalid username. In this case ONLY letters and numbers.
  else if (!preg_match("/^[a-zA-Z0-9]*$/", $username)) {
   echo "<script> alert('Invalid username!!');window.location='../provideruser.php?email=".$email."'</script>";
         exit();
  }
 
  // We check if the repeated password is NOT the same.
  else if ($password !== $passwordRepeat) {
   header("Location: ../provideruser.php?error=passwordcheck&username=".$username."&email=".$email.""); 
          exit();
  }
  else {
   ...
   echo "<script> alert('Error on your database!!');window.location='../provideruser.php'</script>";
         exit();
  }
    else {
      ...
      if ($resultCount > 0) {
     header("Location: ../provideruser.php?error=usertaken"); 
          exit();
      }
      else {
        ...
        $sql = "INSERT INTO users (username, email, password, provider_id) VALUES (?, ?, ?, $provider_id);";
        // Here we initialize a new statement using the connection from the dbh.inc.php file.
        $stmt = mysqli_stmt_init($conn);
        // Then we prepare our SQL statement AND check if there are any errors with it.
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          // If there is an error we send the user back to the signup page.
        echo "<script> alert('Error occured, Please try again!!');window.location='../provideruser.php'</script>";
         exit();
        }
        else {
          ...
          mysqli_stmt_execute($stmt);
          // Lastly we send the user back to the provideruser page with a success message!
          header("Location: ../provideruser.php?provideruser=success"); 
          exit();
  ...
  // If the user tries to access this page an inproper way, we send them back to the provideruser page.
  header("Location: ../provideruser.php");
  exit();
}
